...
//         localStorage.setItem('strategy', strategy); // 存储选中的策略
//     };

//     const handleCustomApologyChange = (event) => {
//         setCustomApology(event.target.value);

//         // 如果输入了自定义道歉信息，则随机选择一个策略
//         if (event.target.value) {
//             const randomStrategy = strategies[Math.floor(Math.random() * strategies.length)];
//             localStorage.setItem('strategy', randomStrategy); // 存储随机选择的策略
//         }

//         localStorage.setItem('message', event.target.value); // 存储自定义道歉信息
//     };
